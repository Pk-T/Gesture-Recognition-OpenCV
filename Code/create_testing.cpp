...
int main()
{
	...
	char imgname[60],imgname2[60];
	int num=1;
	int i,j,len,ll,l,ind;
//	int k[1000];
	int fetr_num=0;
	int g[20][1000];
	IplImage *img1,*img2,*b,*img21,*img_1,*img_2;
	int gray_lvl=10;
	CvSize z;
	CvScalar s;
...
//	FILE *p=popen("ls -l Test/*.png | wc -l","r");
//	if(!p)
...
//	cvNamedWindow("Display",1);
	len=41;
			for(ll=0;ll<len-1;ll++)
		{
			//cout<<"Value of i:"<<i;
			
			sprintf(imgname,"Test/depth%04d.png\0",ll+1);
			sprintf(imgname2,"Test/depth%04d.png\0",ll);//(i+1),k[ll]
