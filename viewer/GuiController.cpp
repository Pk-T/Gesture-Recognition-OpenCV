...
void GuiController :: onRGBDDataUpdated()
{
  ...
  if (m_grab_frames && capturedframes<121){
    m_frame_recorder->saveCurrentFrame(m_last_image);
    capturedframes++;
  }
  else {
      if(capturedframes>=121)
          m_raw_images_window->ui->label_2->setText("Capturing Complete. Press Predict!");
      m_grab_frames = false;
      capturedframes = 0;
      m_frame_recorder->resetFrameIndex();
  }
...
void GuiController::resetCapturedFrames()
{
    capturedframes=0;
    m_frame_recorder->resetFrameIndex();
}
